...
static bool toyota_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->bus == 0U) {
    ...
    if (msg->addr == 0x191U) {
      ...
      // 【依 candy opendbc 逐字對照修改】移除 !toyota_lta 靜態閘門，改為無條件
      // 執行角度變化率/扭矩上限驗證（steer_angle_cmd_checks 等）。理由：這些
      // 驗證函式本身已有物理邊界限制（角速率、扭矩上限），不需要額外依賴「配對
      // 時二選一」這層閘門才能保證安全。此改動逐字對照 candy 目前使用中、已
      // 實際上路測試半年以上的 opendbc（commit 20b651a）。
    ...
    if (msg->addr == 0x2E4U) {
      ...
      // 【依 candy opendbc 逐字對照修改】同上，移除 !toyota_lta 靜態閘門，
      // 無條件執行標準扭矩驗證（steer_torque_cmd_checks 本身已有扭矩上限）。
